...
public class CMMMixerVisual extends SingleAxisRotatingVisual<CMMMechanicalMixerBlockEntity> implements SimpleDynamicVisual {
    private final RotatingInstance mixerHead;
    private final OrientedInstance mixerPole;
    private final CMMMechanicalMixerBlockEntity mixer;

    public CMMMixerVisual(VisualizationContext context, CMMMechanicalMixerBlockEntity blockEntity, float partialTick) {
        super(context, blockEntity, partialTick, Models.partial(AllPartialModels.SHAFTLESS_COGWHEEL));
        this.mixer = blockEntity;

        mixerHead = instancerProvider().instancer(AllInstanceTypes.ROTATING, Models.partial(blockEntity.getPartialModels()[0]))
                .createInstance();
